...
#define toSizeT(t)      (((t)%SIZE_T) != 0)?                   \
                        ((t) + (SIZE_T) - ((t) % (SIZE_T))):t   // TODO
//
...
private:
   ...
   MemRecycleList<T>* getMemRecycleList(size_t n) {
      size_t m = n % R_SIZE;
      // TODO
      if(_recycleList[m].getArrSize() == n)
          return _recycleList + m;

      MemRecycleList<T>* pt = _recycleList + m;
      while(pt->getNextList()){
          pt = pt->getNextList();
          if(pt->getArrSize() == n) return pt;
      }

      pt->setNextList(new MemRecycleList<T>(n));

      return pt->getNextList();
   }
   ...
   T* getMem(size_t t) {
      ...
      if(!_activeBlock->getMem(t, ret)){
          size_t remained = _activeBlock->getRemainSize();
          size_t remainSize;
          if(remained >= S) remainSize = getArraySize(remained);
          else remainSize = 0;
          if(remained != 0){
              MemRecycleList<T>* remainList = getMemRecycleList(remainSize);
              remainList->pushFront((T*)_activeBlock->_ptr);
          }
   ...
   size_t getNumBlocks() const {
      // TODO
      size_t num = 1;
      MemBlock<T>* pt = _activeBlock;
      while(pt->getNextBlock()){
          ++num;
          pt = pt->getNextBlock();
      }
      return num;
