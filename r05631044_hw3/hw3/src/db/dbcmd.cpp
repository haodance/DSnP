...
DBAppendCmd::exec(const string& option)
{
   ...
   if(!this->lexOptions(option, options, 2)) return CMD_EXEC_ERROR;
   else{
       string key = options[0];
       int value;

       if(!isValidVarName(options[0]))
           return this->errorOption(CMD_OPT_ILLEGAL, options[0]);
       
       else if(!myStr2Int(options[1], value))
           return this->errorOption(CMD_OPT_ILLEGAL, options[1]);
...
{  
   // TODO...
   string token;
   if(!this->lexSingleOption(option, token)) return CMD_EXEC_ERROR;

   if(token.size()){
       for(vector<string>::size_type i; i!=dbjson.size(); i++){
